    ...
    if request.method=='GET':
        ...
        for i in range(0, len(data_list), 2):
            if data_list[i][0]==province:
                if data_list[i][1] == "布":
                    data.append({"name":data_list[i][1], "value": float(data_list[i][26-time]) * 10000})
    ...
    if request.method=='GET':
        ...
        for i in range(0, len(data_list), 2):
            if data_list[i][1]==product:
                if product == "布":
                    data.append({"key":data_list[i][0], "value":float(data_list[i][26-time]) * 10000})
    ...
    if request.method=='GET':
        ...
        for i in range(0, len(data_list), 2):
            if data_list[i][0]==province and data_list[i][1]==product:
                if product == "布":
                    for j in range(1, 25):
                        data.append(float(data_list[i][26-j])*10000)
                elif product == "工业锅炉" or product == "金属冶炼设备" or product == "水泥专用设备" or product == "包装专用设备" or product == "大型拖拉机" or product == "中型拖拉机" or product == "大气污染防治设备" or product == "铁路机车":
                    for j in range(1, 25):
                        data.append(float(data_list[i][26-j])/10000)
                else:
